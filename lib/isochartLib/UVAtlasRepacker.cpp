    ...
    size_t iNumRotate)
{
    ...
    {
    	printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__);
        return E_INVALIDARG;
    }
    ...
    {
    	printError(hr, __FILE__, __FUNCTION__, __LINE__);
        return hr;
    }
...
HRESULT CUVAtlasRepacker::Repack()
{
	...
	printProgress(__FILE__, __FUNCTION__, __LINE__, "Pack preparing.");
    ...
    //DPF(3, "Ready\n");
	printProgress(__FILE__, __FUNCTION__, __LINE__, "Ready.");
    ...
    do {
        ...
        std::string message = "Estimated Space Percent = " + std::to_string(double(m_EstimatedSpacePercent * 100.f)) + " %";
        ...
        if (m_OutOfRange)
        {
        	...
        	printProgress(__FILE__, __FUNCTION__, __LINE__, "Current packing is aborted.");
        	printProgress(__FILE__, __FUNCTION__, __LINE__, "Adjusting estimated percent and restart packing.");
        }
    ...
    if (m_bDwIndex)
    {
        ...
        std::string message = "Final space utilization ratio after pack = " + std::to_string(percentOur * 100) + " %";
    	printProgress(__FILE__, __FUNCTION__, __LINE__, message);

        if (m_pPercentOur){ *m_pPercentOur = percentOur; }
    }
    else
    {
    ...
    std::string message = "Final X and Y = " + std::to_string(m_RealHeight) + ", " + std::to_string(m_RealWidth);
	printProgress(__FILE__, __FUNCTION__, __LINE__, message);
...
bool CUVAtlasRepacker::SetCallback(LPISOCHARTCALLBACK pCallback, float Frequency)
{
    ...
    m_callbackSchemer.SetCallback(pCallback,Frequency);
...
void CUVAtlasRepacker::AdjustEstimatedPercent()
{
    ...
    else
    {
        ...
        std::string message = "Unpacked area ratio= " + std::to_string(double(unpackedArea)) + "unpacked charts ratio = " + std::to_string(double(unpackedCharts));
    ...
    if (m_EstimatedSpacePercent <= 0){ m_EstimatedSpacePercent = oldp * 0.9f; }

    m_PixelWidth = sqrtf(m_fChartsTotalArea / (m_EstimatedSpacePercent * m_dwAtlasWidth * m_dwAtlasHeight));
}
...
void CUVAtlasRepacker::ComputeChartsLengthInPixel()
{
    ...
    for (size_t i = 0; i < m_iNumCharts; i++)
    {
        ...
        for (size_t j = 0; j < m_iRotateNum; j++)
        {
            ...
            pPosInfo->basePoint = glm::vec2(pPosInfo->minPoint.x - m_iGutter * m_PixelWidth - adjustX,
                    						pPosInfo->minPoint.y - m_iGutter * m_PixelWidth - adjustY);
...
//        	std::cout << "UV: " << m_UVBoard[size_t(i)][size_t(j)] << std::endl;
        }
    ...
    /*XMStoreFloat4x4(&m_ResultMatrix[index], XMMatrixTranslation(
        ...
        0.0f));*/
    m_ResultMatrix[index] = glm::translate(glm::mat4(1.0f), glm::vec3(m_PixelWidth * m_fromX - m_ChartsInfo[index].PosInfo[0].basePoint.x,
    												 	    		  m_PixelWidth * m_fromY - m_ChartsInfo[index].PosInfo[0].basePoint.y,
																	  0.0f));
...
{
    if (m_dwAtlasHeight / size_t(m_iGutter + 1) * m_dwAtlasWidth / size_t(m_iGutter + 1) <= m_iNumCharts)
    {
        ...
        std::string message1 = "Chart number = " + std::to_string(m_iNumCharts);
        std::string message2 = "Gutter = " + std::to_string(m_iGutter);
        std::string message3 = "User specified atlas : width = " + std::to_string(m_dwAtlasWidth) + ", height = " + std::to_string(m_dwAtlasHeight);
        std::string message4 = "The theoretic maximum charts the atlas can hold is  = " + std::to_string(m_dwAtlasHeight / size_t(m_iGutter + 1) * m_dwAtlasWidth / size_t(m_iGutter + 1));

        printProgress(__FILE__, __FUNCTION__, __LINE__,  "Warning : \nGutter is too large or the atlas resolution is too small.");
        printProgress(__FILE__, __FUNCTION__, __LINE__,  message1);
        printProgress(__FILE__, __FUNCTION__, __LINE__,  message2);
        printProgress(__FILE__, __FUNCTION__, __LINE__,  message3);
        printProgress(__FILE__, __FUNCTION__, __LINE__,  message4);
        printProgress(__FILE__, __FUNCTION__, __LINE__,  "So it is impossible to pack it into user specified atlas.");
...
bool CUVAtlasRepacker::CheckUserInput()
{
    if (!m_pvVertexBuffer)
    {
        ...
        printProgress(__FILE__, __FUNCTION__, __LINE__, "Pack input vertex buffer pointer is nullptr!");
        return false;
    }
    if (m_pvVertexBuffer->size() != m_iNumVertices)
    {
        DPF(0, "Pack input vertex structure should be (x,y,z,u,v)");
        printProgress(__FILE__, __FUNCTION__, __LINE__, "Pack input vertex structure should be (x,y,z,u,v)");
        return false;
    }
    ...
    if (!m_pvIndexBuffer)
    {
        ...
        printProgress(__FILE__, __FUNCTION__, __LINE__, "Pack input face index buffer pointer is nullptr!");
        return false;
    }
        ...
        m_pvIndexBuffer->size() != m_iNumFaces * 3 * sizeof(uint16_t))
    {
        ...
        printProgress(__FILE__, __FUNCTION__, __LINE__, "Pack input face index buffer is neither a uint16_t array nor a uint32_t array");
        return false;
    }
    ...
    m_TexCoordOffset = sizeof(/*XMFLOAT3*/ glm::vec3); // should be glm::vec2 ????????
        ...
        for (uint32_t i = 0; i < m_iNumFaces; i++)
        {
            if (pAB[i] == uint32_t(-1))
            {
                ...
                for (size_t j = 0; j < ab.size(); j++)
                {
                    ...
                    memcpy(&m_NewAdjacentInfo[j * 3 + facestart * 3], &m_AdjacentInfo[3 * ab[j]], sizeof(uint32_t) * 3);
                    ...
                    auto p1 = reinterpret_cast<const /*XMFLOAT2**/ glm::vec2*>(pVB + index1 * m_iNumBytesPerVertex + m_TexCoordOffset);
                    auto p2 = reinterpret_cast<const /*XMFLOAT2**/ glm::vec2*>(pVB + index2 * m_iNumBytesPerVertex + m_TexCoordOffset);
                    auto p3 = reinterpret_cast<const /*XMFLOAT2**/ glm::vec2*>(pVB + index3 * m_iNumBytesPerVertex + m_TexCoordOffset);

                    // find the original 3D coordinates of each vertex
                    auto pp1 = reinterpret_cast<const /*XMFLOAT3**/ glm::vec3*>(pVB + index1 * m_iNumBytesPerVertex);
                    auto pp2 = reinterpret_cast<const /*XMFLOAT3**/ glm::vec3*>(pVB + index2 * m_iNumBytesPerVertex);
                    auto pp3 = reinterpret_cast<const /*XMFLOAT3**/ glm::vec3*>(pVB + index3 * m_iNumBytesPerVertex);
...
void CUVAtlasRepacker::ComputeBoundingBox(
    std::vector</*XMFLOAT2*/ glm::vec2>& Vec,
	/*XMFLOAT2**/ glm::vec2* minV, /*XMFLOAT2**/ glm::vec2* maxV)
{
...
HRESULT CUVAtlasRepacker::PrepareChartsInfo()
{
	...
	glm::mat4 bestMatrix = glm::mat4(1.0f);
    float RotateAngle = 5.0f;

    std::vector</*XMFLOAT2*/ glm::vec2> OutVec;
    ...
    for (uint32_t i = 0; i < m_iNumCharts; i++)
    {
        ...
        {
            float angle = j * RotateAngle / 180.0f * /*XM_PI*/M_PI;
            if (angle > /*XM_PI*/M_PI / 2.0f)
                angle = /*XM_PI*/M_PI / 2.0f;
            //XMMATRIX rotateMatrix = XMMatrixRotationZ(angle);
            glm::mat4 rotateMatrix = glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0.0f, 0.0f, 1.0f));
            ...
            for(unsigned int u = 0; u < m_AttrTable[i].VertexCount; u++)
            {
            	/*
            	 * NOT SURE IF CORRECT
            	 */
//            	glm::vec4 conv = rotateMatrix * glm::vec4(*(&(m_VertexBuffer[m_AttrTable[i].VertexStart].uv) + u*VertexSize), 0.0f, 1.0f);
//            	*(&(OutVec[0]) + u * sizeof(glm::vec2)) = glm::vec2(conv.x, conv.y);
            	glm::vec4 conv = rotateMatrix * glm::vec4(m_VertexBuffer[m_AttrTable[i].VertexStart + u].uv, 0.0f, 1.0f);
            	OutVec[u]= glm::vec2(conv.x, conv.y);
            }

            /*XMFLOAT2*/ glm::vec2 minV(1e10f, 1e10f);
            /*XMFLOAT2*/ glm::vec2 maxV(-1e10f, -1e10f);
        ...
        for(unsigned int u = 0; u < m_AttrTable[i].VertexCount; u++)
        {
//        	glm::vec4 conv = bestMatrix * glm::vec4(*(&(m_VertexBuffer[m_AttrTable[i].VertexStart ].uv) + u*VertexSize), 0.0f, 1.0f);
//        	*(&(m_VertexBuffer[m_AttrTable[i].VertexStart ].uv) + u*VertexSize) = glm::vec2(conv.x, conv.y);
        	glm::vec4 conv = bestMatrix * glm::vec4(m_VertexBuffer[m_AttrTable[i].VertexStart + u].uv, 0.0f, 1.0f);
        	m_VertexBuffer[m_AttrTable[i].VertexStart +u].uv = glm::vec2(conv.x, conv.y);
        }
        ...
        for (size_t j = 0; j < m_iRotateNum; j++)
        {
            float angle = j * /*XM_PI*/M_PI / m_iRotateNum / 2.0f;
            /*XMMATRIX*/glm::mat4 rotateMatrix = /*XMMatrixRotationZ(angle)*/ glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0.0f, 0.0f, 1.0f));
...
//            	*(&(OutVec[0]) + u * sizeof(glm::vec2)) = glm::vec2(conv.x, conv.y);
            ...
            if (j == 0) { m_ChartsInfo[i].maxLength = std::max(maxV.x - minV.x, maxV.y - minV.y); }
            ...
            for (uint32_t k = 0; k < m_AttrTable[i].FaceCount; k++)
            {
                ...
                /*XMFLOAT2&*/ glm::vec2& Vertex1 = *reinterpret_cast</*XMFLOAT2**/ glm::vec2*>(&OutVec[a - indexbase]);
                /*XMFLOAT2&*/ glm::vec2& Vertex2 = *reinterpret_cast</*XMFLOAT2**/ glm::vec2*>(&OutVec[b - indexbase]);
                /*XMFLOAT2&*/ glm::vec2& Vertex3 = *reinterpret_cast</*XMFLOAT2**/ glm::vec2*>(&OutVec[c - indexbase]);
                    ...
                    ((Vertex3.x == Vertex2.x) && (Vertex3.y == Vertex2.y)))
                {
                    ...
                    for (uint32_t t = 0; t < m_AttrTable[i].FaceCount; t++)
                    {
                        ...
                        {
                        	/*XMFLOAT2&*/ glm::vec2& vert1 = *reinterpret_cast</*XMFLOAT2**/ glm::vec2*>(&OutVec[a0 - indexbase0]);
                        	/*XMFLOAT2&*/ glm::vec2& vert2 = *reinterpret_cast</*XMFLOAT2**/ glm::vec2*>(&OutVec[b0 - indexbase0]);
                        	/*XMFLOAT2&*/ glm::vec2& vert3 = *reinterpret_cast</*XMFLOAT2**/ glm::vec2*>(&OutVec[c0 - indexbase0]);
                        ...
                        {
                        	printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__);
                            return E_FAIL;
                        }
                    }
                    break;
...
void CUVAtlasRepacker::PutChart(uint32_t index)
{
    ...
    for (uint32_t i = 0; i < m_iRotateNum; i++)
    {
        ...
        PrepareSpaceInfo(m_currSpaceInfo, m_currChartUVBoard, 0, pPosInfo->numX, 0, pPosInfo->numY, true);
...
//    	std::cout << "OUTOFRANGE\n";
    ...
    /*XMMATRIX*/ glm::mat4 matrixRotate;
    /*matrixRotate = XMMatrixRotationZ(m_triedPutRotation / 180.0f * XM_PI);
    XMStoreFloat2(&(pPosInfo->basePoint), XMVector2TransformCoord(XMLoadFloat2(&(pPosInfo->basePoint)), matrixRotate));
    matrixRotate = XMMatrixRotationZ(m_triedPutRotation / 180.0f * XM_PI +pPosInfo->angle);*/
    matrixRotate = glm::rotate(glm::mat4(1.0f), (float)(m_triedPutRotation / 180.0f * M_PI), glm::vec3(0.0f, 0.0f, 1.0f));

    //printMatrix(matrixRotate);

    glm::vec4 tempMat = matrixRotate * glm::vec4(pPosInfo->basePoint, 0.0f, 1.0f);
    pPosInfo->basePoint = glm::vec2(tempMat.x, tempMat.y);
    matrixRotate = glm::rotate(glm::mat4(1.0f), (float)(m_triedPutRotation / 180.0f * M_PI + pPosInfo->angle), glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    case 0:
        ...
        //transMatrix = XMMatrixTranslation(m_PixelWidth * m_chartFromX - pPosInfo->basePoint.x,m_PixelWidth * m_chartFromY - pPosInfo->basePoint.y, 0.0f);
        transMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(m_PixelWidth * m_chartFromX - pPosInfo->basePoint.x,
        											  	  	    m_PixelWidth * m_chartFromY - pPosInfo->basePoint.y,
																0.0f));
    ...
    case 90:
        ...
        //transMatrix = XMMatrixTranslation(m_PixelWidth * m_chartToX - pPosInfo->basePoint.x,m_PixelWidth * m_chartFromY - pPosInfo->basePoint.y, 0.0f);
        transMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(m_PixelWidth * m_chartToX - pPosInfo->basePoint.x,
        											  	  	    m_PixelWidth * m_chartFromY - pPosInfo->basePoint.y,
																0.0f));
        break;
    case 180:
        ...
        //transMatrix = XMMatrixTranslation(m_PixelWidth * m_chartToX - pPosInfo->basePoint.x,m_PixelWidth * m_chartToY - pPosInfo->basePoint.y, 0.0f);
        transMatrix = glm::translate(glm::mat4(1.0f),glm::vec3(m_PixelWidth * m_chartToX - pPosInfo->basePoint.x,
        											 	 	   m_PixelWidth * m_chartToY - pPosInfo->basePoint.y,
															   0.0f));
        break;
    case 270:
        for (int i = m_chartFromY; i < m_chartToY; i++)
        ...
        //transMatrix = XMMatrixTranslation(m_PixelWidth * m_chartFromX - pPosInfo->basePoint.x,m_PixelWidth * m_chartToY - pPosInfo->basePoint.y, 0.0f);
        transMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(m_PixelWidth * m_chartFromX - pPosInfo->basePoint.x,
        													    m_PixelWidth * m_chartToY - pPosInfo->basePoint.y,
																0.0f));
        break;
    ...
    m_ResultMatrix[index] = transMatrix * matrixRotate;
...
{
    /*XMMATRIX*/ glm::mat4 transMatrix, scalMatrix, matrix;

    //transMatrix = XMMatrixTranslation(-m_PixelWidth * (m_fromX + m_iGutter), -m_PixelWidth * (m_fromY + m_iGutter), 0.0f);
    //scalMatrix = XMMatrixScaling(1.0f / m_PixelWidth / m_NormalizeLen, 1.0f / m_PixelWidth / m_NormalizeLen, 0.0f);
    transMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-m_PixelWidth * (m_fromX + m_iGutter),
    													    -m_PixelWidth * (m_fromY + m_iGutter),
															0.0f));
    scalMatrix = glm::scale(glm::mat4(1.0f), glm::vec3(1.0f / m_PixelWidth / m_NormalizeLen,
    												   1.0f / m_PixelWidth / m_NormalizeLen,
													   1.0f)); // scale 0.0 or 1.0 ???????????????ß
    ...
    for (size_t i = 0; i < m_iNumCharts; i++)
    {
        if (m_ChartsInfo[i].valid)
        {
        	...
        	matrix = scalMatrix * transMatrix * m_ResultMatrix[i];
        	//printMatrix(matrix);
            for(unsigned int u = 0; u < m_AttrTable[i].VertexCount; u++)
            {
//            	glm::vec4 conv = matrix * glm::vec4(*(&(m_VertexBuffer[m_AttrTable[i].VertexStart].uv) + u*VertexSize), 0.0f, 1.0f);
//            	*(&(m_VertexBuffer[m_AttrTable[i].VertexStart].uv) + u*VertexSize) = glm::vec2(conv.x, conv.y);
            	glm::vec4 conv = matrix * glm::vec4(m_VertexBuffer[m_AttrTable[i].VertexStart+u].uv, 0.0f, 1.0f);
            	m_VertexBuffer[m_AttrTable[i].VertexStart+u].uv = glm::vec2(conv.x, conv.y);
            }
        }
        else
...
float CUVAtlasRepacker::GetChartArea(uint32_t index) const
{
    ...
    {
        const /*XMFLOAT2**/ glm::vec2* p1 = &m_VertexBuffer[m_IndexPartition[m_IndexBuffer[3 * i]]].uv;
        const /*XMFLOAT2**/ glm::vec2* p2 = &m_VertexBuffer[m_IndexPartition[m_IndexBuffer[3 * i + 1]]].uv;
        const /*XMFLOAT2**/ glm::vec2* p3 = &m_VertexBuffer[m_IndexPartition[m_IndexBuffer[3 * i + 2]]].uv;
...
float CUVAtlasRepacker::GetTotalArea() const
{
    ...
    for (size_t i = 0; i < m_iNumFaces; i++)
    {
        auto p1 = reinterpret_cast<const /*XMFLOAT2**/ glm::vec2*>(pVB + *reinterpret_cast<const T*>(pIB + 3 * i * sizeof(T))* m_iNumBytesPerVertex + m_TexCoordOffset);
        auto p2 = reinterpret_cast<const /*XMFLOAT2**/ glm::vec2*>(pVB + *reinterpret_cast<const T*>(pIB + (3 * i + 1) * sizeof(T))* m_iNumBytesPerVertex + m_TexCoordOffset);
        auto p3 = reinterpret_cast<const /*XMFLOAT2**/ glm::vec2*>(pVB + *reinterpret_cast<const T*>(pIB + (3 * i + 2) * sizeof(T))* m_iNumBytesPerVertex + m_TexCoordOffset);
...
bool CUVAtlasRepacker::DoTessellation(uint32_t ChartIndex, size_t AngleIndex)
{
    ...
    for (size_t i = 0; i < pPosInfo->edges.size(); i++)
    {
        ...
        /*XMFLOAT2**/ glm::vec2* p1 = &(pPosInfo->edges[i].p1);
        glm::vec2* p2 = &(pPosInfo->edges[i].p2);
        glm::vec2* pMinP = &(pPosInfo->edges[i].minP);
        glm::vec2* pMaxP = &(pPosInfo->edges[i].maxP);
        ...
        /*
         * ERROR in HERE: fromX and fromY out of range!!!!!!!!!!!!!
         */
        int m, n;
        if (toX - fromX <= 1 && toY - fromY <= 1)
        {
//        	std::cout << "TESTB" << std::endl;
//        	std::cout << "fromX: " << fromX << std::endl;
//        	std::cout << "fromY: " << fromY << std::endl;
//        	std::cout << "m_iGutter: " << m_iGutter << std::endl;
//        	std::cout << "m_currChartUVBoard.size(): " << m_currChartUVBoard.size() << std::endl;
//        	if((unsigned int)fromX < 0){
//        		std::cout << "fromX: " << fromX << std::endl;
//        		printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "fromX is OutOfRange.");
//        	}
