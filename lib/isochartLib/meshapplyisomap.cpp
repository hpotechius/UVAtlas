    ...
    size_t& dwLandmarkNumber)
{
    ...
    std::string message = "Total landmark count is : " + std::to_string(dwLandmarkNumber);
    ...
    ISOCHARTVERTEX* pVertexC) const
{
    ...
    v[0] = glm::vec4(*(m_baseInfo.pVertPosition+pVertexB->dwIDInRootMesh), 0.0f) - glm::vec4(*(m_baseInfo.pVertPosition+pVertexC->dwIDInRootMesh), 0.0f);
    v[1] = glm::vec4(*(m_baseInfo.pVertPosition+pVertexA->dwIDInRootMesh), 0.0f) - glm::vec4(*(m_baseInfo.pVertPosition+pVertexC->dwIDInRootMesh), 0.0f);

    /*float a = XMVectorGetX(XMVector3Length(v[0]));
    float b = XMVectorGetX(XMVector3Length(v[1]));*/
    float a = glm::length(glm::vec3(v[0].x,v[0].y,v[0].z));
    float b = glm::length(glm::vec3(v[1].x,v[1].y,v[1].z));
    float c = a * b;
    ...
    float fCosTheta = glm::dot(glm::vec3(v[0].x,v[0].y,v[0].z), glm::vec3(v[1].x,v[1].y,v[1].z)) / c;

    /*v[2] = XMVector3Cross(v[0], v[1]);
    float fSinTheta = XMVectorGetX(XMVector3Length(v[2])) / c;*/
    v[2] = glm::vec4(glm::cross(glm::vec3(v[0].x,v[0].y,v[0].z), glm::vec3(v[1].x,v[1].y,v[1].z)), 0.0f);
    float fSinTheta = glm::length(glm::vec3(v[2].x,v[2].y,v[2].z)) / c;
                                ...
                                // store UV coordinate in vertex
{
    ...
    for (size_t i = 0; i < m_dwFaceNumber; i++)
    {
        /*XMFLOAT3*/ glm::vec3 vec1(m_pVerts[pFace->dwVertexID[1]].uv.x - m_pVerts[pFace->dwVertexID[0]].uv.x,
            m_pVerts[pFace->dwVertexID[1]].uv.y - m_pVerts[pFace->dwVertexID[0]].uv.y,
            0);

        /*XMFLOAT3*/ glm::vec3 vec2(m_pVerts[pFace->dwVertexID[2]].uv.x - m_pVerts[pFace->dwVertexID[0]].uv.x,
