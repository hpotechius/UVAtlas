...
namespace
{
    ...
    private:
        ...
        const /*XMFLOAT3**/ glm::vec3* m_pVert3dPos;
    public:
        CBoundaryIter(
            uint32_t dwInit,
            ISOCHARTVERTEX* pVerts,
            const /*XMFLOAT3**/ glm::vec3* pVert3dPos)
            :m_dwInit(dwInit),
            m_dwPrev(dwInit),
            m_dwCurr(dwInit),
            m_pVerts(pVerts),
        ...
        {
            //const glm::vec3 *p1 = m_pVert3dPos[m_pVerts[m_dwCurr].dwIDInRootMesh];
            //const glm::vec3 *p2 = m_pVert3dPos[m_pVerts[m_dwPrev].dwIDInRootMesh];
            const /*XMFLOAT3**/ glm::vec3* p1 = &m_pVert3dPos[m_pVerts[m_dwCurr].dwIDInRootMesh];
            const /*XMFLOAT3**/ glm::vec3* p2 = &m_pVert3dPos[m_pVerts[m_dwPrev].dwIDInRootMesh];
...
HRESULT CIsochartMesh::BarycentricParameterization(bool& bIsOverLap)
{
    HRESULT hr = S_OK;
    ...
    GenerateVertexMap(vertMap,dwBoundaryCount,dwInternalCount);
    /*FAILURE_GOTO_END(
        GenerateVertexMap(
            vertMap,
            dwBoundaryCount,
    ...
    GenerateBoundaryCoord(boundTable,dwBoundaryCount,vertMap);
    ...
    InitializeBarycentricEquation(A,BU,BV,boundTable,vertMap);
    /*FAILURE_GOTO_END(
    ...
    CSparseMatrix<double>::ConjugateGradient(U,A,BU,BC_MAX_ITERATION,1e-8,nIterCount);
    /*FAILURE_GOTO_END(
    ...
    CSparseMatrix<double>::ConjugateGradient(V,A,BV,BC_MAX_ITERATION,1e-8,nIterCount);
    /*nIterCount = 0;
    FAILURE_GOTO_END(
        (false != CSparseMatrix<double>::ConjugateGradient(
            V,
    ...
    AssignBarycentricResult(U,V,boundTable,vertMap);
    /*FAILURE_GOTO_END(
    ...
    // 5. Check Results
    CheckLinearEquationParamResult(bIsOverLap);
    /*FAILURE_GOTO_END(
