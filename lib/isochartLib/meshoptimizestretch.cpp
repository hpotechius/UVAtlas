...
namespace Isochart
{
    ...
    struct VERTOPTIMIZEINFO
    {
        ...
        /*XMFLOAT2*/ glm::vec2 start;
        /*XMFLOAT2*/ glm::vec2 end;
        float fStartStretch;
        float fEndStretch;
    ...
    for (size_t ii = 0; ii < m_dwFaceNumber; ii++)
    {
        ...
        if (fStretch == INFINITE_STRETCH)
        {
           // DPF(0, "Can not opimize scale all chart, some face has infinite stretch");
            printProgress(__FILE__, __FUNCTION__, __LINE__, "Can not opimize scale all chart, some face has infinite stretch.");
            goto LEnd;
        }
    ...
    bool& bSucceed)
{
    ...
    if (bCanOptimize)
    {
        ...
        bSucceed = ((optimizeInfo.fInfinitFacesArea / m_fChart3DArea) <= m_baseInfo.fOverturnTolerance);
    }
    ...
    if (!bSucceed)
    {
        /*DPF(1, "Infinite Optimize failed, %zu Internal infinite vertices,%zu boundary vert",
            ...
            dwBoundaryInfFaces);*/
        std::string message = "Infinite Optimize failed, " + std::to_string(optimizeInfo.dwInfinitStretchVertexCount - dwBoundaryInfFaces)
        						+ " Internal infinite vertices, " + std::to_string(dwBoundaryInfFaces) + " boundary vert";
    ...
    float* pGeoM) const
{
    ...
    {
    	/*XMFLOAT2**/ glm::vec2* pCanonicalUV = m_baseInfo.pFaceCanonicalUVCoordinate + pFace->dwIDInRootMesh * 3;
    ...
    float& f2D) const
{
    ...
    else
    {
        ...
        glm::vec4 vSs = glm::vec4(Ss, 0.0f);
        glm::vec4 vSt = glm::vec4(St, 0.0f);
        float a = glm::dot(glm::vec3(vSs.x,vSs.y,vSs.z), glm::vec3(vSs.x,vSs.y,vSs.z));
        float c = glm::dot(glm::vec3(vSt.x,vSt.y,vSt.z), glm::vec3(vSt.x,vSt.y,vSt.z));
    ...
    float& f2D) const
{
    ...
    else if (f2D < 0 || (f2D < ISOCHART_ZERO_EPS2 && f2D < f3D / 2))
    {
        return INFINITE_STRETCH;
    }
    ...
    else
    {
        ...
        float a = glm::dot(glm::vec3(vSs.x,vSs.y,vSs.z), glm::vec3(vSs.x,vSs.y,vSs.z));
        float c = glm::dot(glm::vec3(vSt.x,vSt.y,vSt.z), glm::vec3(vSt.x,vSt.y,vSt.z));
        float b = glm::dot(glm::vec3(vSs.x,vSs.y,vSs.z), glm::vec3(vSt.x,vSt.y,vSt.z));
    ...
    VERTOPTIMIZEINFO& vertInfo)
{
    ...
    for (size_t i = 0; i < dwAdjacentVertexCount; i++)
    {
        ...
        middle = pVertex1->uv * CONSERVATIVE_OPTIMIZE_FACTOR + vertInfo.center * (1 - CONSERVATIVE_OPTIMIZE_FACTOR);
        ...
        if (fTempStretch < vertInfo.fEndStretch)
        {
            ...
            memcpy(vertInfo.pfEndFaceStretch, vertInfo.pfWorkStretch, sizeof(float) * dwAdjacentFaceCount);
            vertInfo.end = middle;
        }
    }
    ...
    {
        pVertex1 = m_pVerts + pOptimizeVertex->vertAdjacent[i];
        float fTemp = CaculateUVDistanceSquare(pVertex1->uv, vertInfo.center);
    ...
    VERTOPTIMIZEINFO& vertInfo)
{
    ...
    /*XMFLOAT2*/ glm::vec2 originalEnd = vertInfo.end;
    ...
    while (iteration < optimizeInfo.dwRandOptOneVertTimes)
    {
        ...
        float fAngle = rand() * 2.f * /*XM_PI*/M_PI / RAND_MAX;
        vertInfo.end.x = vertInfo.center.x + vertInfo.fRadius * cosf(fAngle);
        vertInfo.end.y = vertInfo.center.y + vertInfo.fRadius * sinf(fAngle);
        ...
        {
            LimitVertexToBoundingBox(vertInfo.end, optimizeInfo.minBound, optimizeInfo.maxBound, vertInfo.end);
        }
        ...
        float fDiffernece = CaculateUVDistanceSquare(vertInfo.start, vertInfo.end);
...
float CIsochartMesh::GetFaceAreaAroundVertex(const ISOCHARTVERTEX* pOptimizeVertex, /*XMFLOAT2&*/ glm::vec2& newUV) const
{
...
void CIsochartMesh::LimitVertexToBoundingBox(
    const /*XMFLOAT2&*/ glm::vec2& end,
    const /*XMFLOAT2&*/ glm::vec2& minBound,
    const /*XMFLOAT2&*/ glm::vec2& maxBound,
	/*XMFLOAT2&*/ glm::vec2& result)
{
...
void CIsochartMesh::UpdateOptimizeResult(
    CHARTOPTIMIZEINFO& optimizeInfo,
    ISOCHARTVERTEX* pOptimizeVertex,
	/*XMFLOAT2&*/ glm::vec2& vertexNewCoordinate,
    ...
    ISOCHARTFACE* pFace)
{
    if (bForSignal)
    {
        ...
        /*XMFLOAT2**/ glm::vec2* p = m_baseInfo.pFaceCanonicalUVCoordinate + m_pFaces->dwIDInRootMesh * 3;
        ...
        std::string message1 = "New Area " + std::to_string(double(fNew2DArea));
        //std::string message2 = "Theory Stretch " + std::to_string(double(m_fParamStretchL2)) + ", New Stretch " + std::to_string(double(fNewStretch));
        printProgress(__FILE__, __FUNCTION__, __LINE__, message1);
        //printProgress(__FILE__, __FUNCTION__, __LINE__, message2);
    }
    else
    {
    	/*XMFLOAT3*/ glm::vec3 axis[2];
