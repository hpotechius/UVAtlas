...
namespace Isochart
{
    ...
    struct PACKINGINFO
    {
        ...
        /*XMFLOAT2**/ glm::vec2* pVertUV;

        // The uv coordinates of chart when its left-bottom corner be moved to origin
        /*XMFLOAT2**/ glm::vec2* pStandardUV;

        /*XMFLOAT2**/ glm::vec2* pStandardVirtualCorner;
...
namespace
{
    ...
    )
    {
        ...
        {
            packingInfo.pStandardVirtualCorner = new (std::nothrow) /*XMFLOAT2[4]*/glm::vec2[4];
            if (!packingInfo.pStandardVirtualCorner)
            {
                return E_OUTOFMEMORY;
            }
    ...
    inline void UpdateMinMaxVertex(
        /*XMFLOAT2&*/ glm::vec2& currentVertex,
		/*XMFLOAT2&*/ glm::vec2& minVec,
		/*XMFLOAT2&*/ glm::vec2& maxVec)
    {
    ...
    void AdjustCornerBorder(
        ...
        size_t dwVertNumber)
    {
    	/*XMFLOAT2*/ glm::vec2 minVec(FLT_MAX, FLT_MAX);
    	/*XMFLOAT2*/ glm::vec2 maxVec(-FLT_MAX, -FLT_MAX);

        for (size_t ii = 0; ii < dwVertNumber; ii++){ UpdateMinMaxVertex(pChartVert[ii].uv, minVec, maxVec); }
    ...
    inline static VertexLocation CalculateVertexLocationToBorder(
        ...
        size_t dwBorderStart,
        size_t dwBorderEnd,
        /*XMFLOAT2&*/ glm::vec2& point,// a vertex
        ...
        Axis TangentAxis)
    {
        ...
        else
        {
        	...
        	glm::vec4 vBiasVector = glm::vec4(aBorder[i]->uv, 0.0f, 0.0f) - glm::vec4(aBorder[i-1]->uv, 0.0f, 0.0f);
        	glm::vec2 biasVector = glm::vec2(vBiasVector.x,vBiasVector.y); // @suppress("Symbol is not resolved")
            ...
            else
            {
                ...
                //fExtraDistance = fGutter * fabsf(XMVectorGetX(XMVector2Length(vBiasVector)) / VECTOR_ITEM(&biasVector, TangentAxis));
                fExtraDistance = fGutter * fabsf(glm::length(glm::vec2(vBiasVector.x,vBiasVector.y)) / VECTOR_ITEM(&biasVector, TangentAxis));
            }
        }
    ...
    inline static void RotateVertexAroundCenter(
        /*XMFLOAT2&*/ glm::vec2& vertexOut,
        const /*XMFLOAT2&*/ glm::vec2& vertexIn,
        float fcenterX,
        float fcenterY,
        float fSin,
        float fCos)
        ...
        float& fDotValue)
    {
        ...
        do
        {
            ...
            /*XMVECTOR vv1 = XMVectorSet(
                ...
                0, 0);*/
            glm::vec4 vv1 = glm::vec4(pVertex1->uv.x - pOrigin->uv.x, pVertex1->uv.y - pOrigin->uv.y, 0.0f, 0.0f);

            /*XMVECTOR vv2 = XMVectorSet(
                pVertex2->uv.x - pOrigin->uv.x,
                pVertex2->uv.y - pOrigin->uv.y,
                0, 0);*/
            glm::vec4 vv2 = glm::vec4(pVertex2->uv.x - pOrigin->uv.x, pVertex2->uv.y - pOrigin->uv.y, 0.0f, 0.0f);

            //float f1 = XMVectorGetX(XMVector3LengthSq(vv1));
            //float f2 = XMVectorGetX(XMVector3LengthSq(vv2));
            float f1 = glm::pow(glm::length(glm::vec3(vv1.x,vv1.y,vv1.z)), 2);
            float f2 = glm::pow(glm::length(glm::vec3(vv2.x,vv2.y,vv2.z)), 2);
            ...
            else
            {
            	...
            	vv1 = glm::vec4(glm::normalize(glm::vec3(vv1.x,vv1.y,vv1.z)), 0.0f);
            	vv2 = glm::vec4(glm::normalize(glm::vec3(vv2.x,vv2.y,vv2.z)), 0.0f);
            	fDotValue = glm::dot(glm::vec3(vv1.x,vv1.y,vv1.z), glm::vec3(vv2.x,vv2.y,vv2.z));
            	glm::vec3 v1, v2;
            	v1 = glm::vec3(vv1.x,vv1.y,vv1.z);
            	v2 = glm::vec3(vv2.x,vv2.y,vv2.z);
    ...
    inline static void MoveChartToNewPosition(
        ...
        const /*XMFLOAT2**/ glm::vec2* pOrigUV,
        Axis TangentAxis,
        Axis RadialAxis,
        float fTangentDelta,
        float fRadialDelta,
        ...
        float& fAreaLost)
    {
        ...
        /*XMFLOAT2**/ glm::vec2* pOrigUV = pPackingInfo->pStandardUV;
    ...
    inline void UpdateAreaLostInfo(
		...
		/*XMFLOAT2*/ glm::vec2 dirOrg[],
		/*XMFLOAT2&*/ glm::vec2& currentOrg,
    ...
    CCallbackSchemer& callbackSchemer)
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Area lost rate is : ...");
    ...
    ATLASINFO& atlasInfo)
{
    ...
    {
        float fAngle = ii * 2.f * /*XM_PI*/ M_PI / CHART_ROTATION_NUMBER;
    ...
    std::string message = "Pixel Length is " + std::to_string(double(atlasInfo.fPixelLength));
...
HRESULT CIsochartMesh::CreatePackingInfoBuffer()
{
    ...
    m_pPackingInfo->pVertUV = new (std::nothrow) glm::vec2[m_dwVertNumber];
    m_pPackingInfo->pStandardUV = new (std::nothrow) glm::vec2[m_dwVertNumber];
...
void CIsochartMesh::AlignUVWithLongestAxis() const
{
    ...
    /*XMFLOAT2*/ glm::vec2 minVec;
    /*XMFLOAT2*/ glm::vec2 maxVec;
    CalculateChartMinimalBoundingBox(CHART_ROTATION_NUMBER,minVec,maxVec);
    ...
    pVertex1 = m_pVerts;
    /*XMFLOAT2**/ glm::vec2* pUV = m_pPackingInfo->pVertUV;
    ...
    size_t dwIteration)
{
    ...
    /*XMFLOAT2*/ glm::vec2 dirOrg[PACKING_DIRECTION_NUMBER];
    /*XMFLOAT2*/ glm::vec2 newOrgin;
    ...
    */
    ...
    else
    {
        ...
        if (dwDirMinRotationId[dwPackDirection] == INVALID_INDEX)
        {
            ...
            std::string message1 = "2d area " + std::to_string(double(pChart->m_fChart2DArea));
            std::string message2 = "3d area " + std::to_string(double(pChart->m_fChart3DArea));
            std::string message3 = "Face number " + std::to_string(double(pChart->m_dwFaceNumber));
            std::string message4 = "Vert number " + std::to_string(double(pChart->m_dwVertNumber));
            ...
            for (size_t ii = 0; ii < pChart->m_dwVertNumber; ii++)
            {
                ...
                std::string message5 = "(" + std::to_string(double(pChart->m_pVerts[ii].uv.x)) + ", " + std::to_string(double(pChart->m_pVerts[ii].uv.y)) + ")";
                printProgress(__FILE__, __FUNCTION__, __LINE__, message5);
            }
        }
    ...
    ATLASINFO& atlasInfo)
{
        ...
        dwRotationCount++)
    {
        ...
        if (!bCanDecide1 || !bCanDecide2)
        {
            ...
            printProgress(__FILE__, __FUNCTION__, __LINE__, "Setup corner boundaries.");
            FAILURE_RETURN(
    ...
    bool& bCanDecide)
{
        ...
        || (!bFirstBorderOutside && bSecondBorderInSide))
    {
        ...
        std::string message = "Dot value 1 = " + std::to_string(double(fDotValue1)) + ", Dot value 2 = " + std::to_string(double(fDotValue2));
        printProgress(__FILE__, __FUNCTION__, __LINE__, message);
    ...
    try
    {
        ...
        while (pVertex != pEndVertex)
        {
            ...
            for (size_t j = 0; j < pVertex->edgeAdjacent.size(); j++)
            {
                ...
                if (pTempEdge->bIsBoundary && pTempEdge != pBoundaryEdge)
                {
                    if (pScanEdge)
                    {
                        ...
                        std::string message = "Vertex " + std::to_string( pVertex->dwIDInRootMesh) + " has more than 2 boundary edges leaving it.";
            ...
            if (pVertex == pStartVertex)
            {
                ...
                printError(ERROR_INVALID_DATA,__FILE__, __FUNCTION__, __LINE__, "Chart has more than 2 boundaries.");
                //return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
                return ERROR_INVALID_DATA;
            }
    ...
    size_t dwRotationId)
{
    ...
    /*XMFLOAT2*/ glm::vec2 minVector(FLT_MAX, FLT_MAX);
    /*XMFLOAT2*/ glm::vec2 maxVector(-FLT_MAX, -FLT_MAX);
    ...
    ATLASINFO& atlasInfo)
{
    ...
    if (atlasInfo.fBoxRight - atlasInfo.fBoxLeft >= (atlasInfo.fBoxTop - atlasInfo.fBoxBottom) * atlasInfo.fWidthHeightRatio)
    {
    ...
    for (size_t i = 0; i < chartList.size(); i++)
    {
        ...
        for (size_t j = 0; j < pChart->m_dwVertNumber; j++)
        {
            ...
            //std::cout << "TESTX: " << pVertex->uv.x << std::endl;

            assert(__finite(double(pVertex->uv.x)));
            assert(__finite(double(pVertex->uv.y)));
    ...
    ISOCHARTMESH_ARRAY& chartList)
{
    ...
    for (size_t i = 0; i < chartList.size(); i++)
    {
        ...
        assert(__finite(double(pChart->m_fParamStretchL2)) != 0);

        fTotal += IsochartSqrtf((pChart->m_fParamStretchL2 + ShiftError) * pChart->m_fChart2DArea);
    }
    ...
    for (size_t i = 0; i < chartList.size(); i++)
    {
        ...
        {
            fScale = IsochartSqrtf((pChart->m_fParamStretchL2 + ShiftError) / pChart->m_fChart2DArea) / fTotal;
            fScale *= fTotal2DArea;
            fScale2 = fScale;
