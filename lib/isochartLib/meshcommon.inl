...
namespace Isochart
{
    ...
    inline float CIsochartMesh::CaculateUVDistanceSquare(
        /*DirectX::XMFLOAT2&*/ glm::vec2& v0,
        /*DirectX::XMFLOAT2&*/ glm::vec2& v1) const
    {
    ...
    inline float CIsochartMesh::CalculateUVFaceArea(
        /*DirectX::XMFLOAT2&*/ glm::vec2 &v0,
        /*DirectX::XMFLOAT2&*/ glm::vec2 &v1,
        /*DirectX::XMFLOAT2&*/ glm::vec2 &v2) const
    {
	...
	glm::vec4 pv0 = glm::vec4(*(m_baseInfo.pVertPosition+v0.dwIDInRootMesh), 0.0f);
	glm::vec4 pv1 = glm::vec4(*(m_baseInfo.pVertPosition+v1.dwIDInRootMesh), 0.0f);
	glm::vec4 v2 = pv1 - pv0;
	return glm::length(glm::vec3(v2.x, v2.y, v2.z));
    ...
    inline void CIsochartMesh::Vertex3DTo2D(
        ...
        const /*DirectX::XMFLOAT3**/ glm::vec3 *pOrg,
        const /*DirectX::XMFLOAT3**/ glm::vec3 *p3D,
        /*DirectX::XMFLOAT2**/ glm::vec2 *p2D)
    {
        //using namespace DirectX;
        glm::vec3* pAxisX = m_baseInfo.pFaceCanonicalParamAxis + 2 * dwFaceIDInRootMesh;
        glm::vec3* pAxisY = pAxisX + 1;
        glm::vec4 tempVector = glm::vec4(*p3D, 0.0f) - glm::vec4(*pOrg, 0.0f);
        p2D->x = glm::dot(glm::vec3(tempVector.x,tempVector.y,tempVector.z), *pAxisX);
        p2D->y = glm::dot(glm::vec3(tempVector.x,tempVector.y,tempVector.z), *pAxisY);
    ...
    inline float CIsochartMesh::CalculateSignalLengthOnOneFace(
        /*DirectX::XMFLOAT3**/ glm::vec3 *p3D0,
        /*DirectX::XMFLOAT3**/ glm::vec3 *p3D1,
        uint32_t dwFaceID)
    {
        //using namespace DirectX;
        ...
        ISOCHARTVERTEX* pVertex = m_pVerts + pFace->dwVertexID[0];

        glm::vec3* pOrg = m_baseInfo.pVertPosition + pVertex->dwIDInRootMesh;
        //XMFLOAT3* pOrg = m_baseInfo.pVertPosition + pVertex->dwIDInRootMesh;

        /*XMFLOAT2*/ glm::vec2 v2d0;
        /*XMFLOAT2*/ glm::vec2 v2d1;

        Vertex3DTo2D(pFace->dwIDInRootMesh, pOrg, p3D0, &v2d0);

        Vertex3DTo2D(pFace->dwIDInRootMesh, pOrg, p3D1, &v2d1);
        ...
        uint32_t dwAdjacentFaceID1)
    {
        ...
        fLength0 = CalculateSignalLengthOnOneFace(p3D0, p3D1, dwAdjacentFaceID0);
        ...
        {
            float fLength1;
            fLength1 = CalculateSignalLengthOnOneFace( p3D0, p3D1, dwAdjacentFaceID1);
    ...
    inline float CIsochartMesh::CalculateEdgeSignalLength(ISOCHARTEDGE& edge)
    {
        ...
        assert((INVALID_FACE_ID == edge.dwFaceID[1] && edge.bIsBoundary) || (INVALID_FACE_ID != edge.dwFaceID[1] || edge.bIsBoundary));

        float fTestLength = 0;
        glm::vec3 *pv0 = m_baseInfo.pVertPosition + m_pVerts[edge.dwVertexID[0]].dwIDInRootMesh;
        glm::vec3 *pv1 = m_baseInfo.pVertPosition + m_pVerts[edge.dwVertexID[1]].dwIDInRootMesh;        
	//DirectX::XMFLOAT3* pv0 = m_baseInfo.pVertPosition + m_pVerts[edge.dwVertexID[0]].dwIDInRootMesh;
        //DirectX::XMFLOAT3* pv1 = m_baseInfo.pVertPosition + m_pVerts[edge.dwVertexID[1]].dwIDInRootMesh;

        fTestLength = CalculateEdgeSignalLength( pv0, pv1, edge.dwFaceID[0], edge.dwFaceID[1]);
    ...
    inline void CIsochartMesh::CalculateChartEdgeLength()
    {
        for (size_t i = 0; i < m_dwEdgeNumber; i++)
        {
            ...
            edge.fLength = CalculateVextexDistance( m_pVerts[edge.dwVertexID[0]], m_pVerts[edge.dwVertexID[1]]);
    ...
    inline void Rotate2DPoint(
        /*DirectX::XMFLOAT2&*/ glm::vec2 &uvOut,
        const /*DirectX::XMFLOAT2&*/ glm::vec2 &uvIn,
        const /*DirectX::XMFLOAT2&*/ glm::vec2 &center,
        float fSin,
        float fCos)
    {
        float x = (uvIn.x - center.x) * fCos - (uvIn.y - center.y) * fSin;
        float y = (uvIn.x - center.x) * fSin + (uvIn.y - center.y) * fCos;
    ...
    inline void CIsochartMesh::RotateChart(const /*DirectX::XMFLOAT2&*/ glm::vec2 &center, float fAngle) const
    {
        ...
        {
            Rotate2DPoint(pVertex->uv, pVertex->uv, center, fSin, fCos);
            pVertex++;
        }
    }
    ...
    inline void CIsochartMesh::GetRotatedChartBoundingBox(
        const /*DirectX::XMFLOAT2&*/ glm::vec2 &center,
        float fAngle,
        /*DirectX::XMFLOAT2&*/ glm::vec2 &minBound,
        /*DirectX::XMFLOAT2&*/ glm::vec2 &maxBound) const
    {
        minBound.x = minBound.y = FLT_MAX;
        maxBound.x = maxBound.y = -FLT_MAX;

        /*DirectX::XMFLOAT2*/ glm::vec2 tempCoordinate;
        ISOCHARTVERTEX* pVertex = m_pVerts;
        ...
        /*DirectX::XMFLOAT2&*/ glm::vec2& maxBound) const
    {
        ...
        /*XMFLOAT2*/ glm::vec2 tempMinBound;
        /*XMFLOAT2*/ glm::vec2 tempMaxBound;
        ...
        {
            const /*XMFLOAT2&*/ glm::vec2 &uv = m_pVerts[ii].uv;
        ...
        {
            float fAngle = dwRotID * /*XM_PI*/ M_PI / (dwRotationCount * 2);
