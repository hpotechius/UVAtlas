...
HRESULT CIsochartEngine::CreateEngineMutex()
{
	...
	if (pthread_mutex_init(&m_hMutex, NULL) != 0) {
		printError(S_FALSE,__FILE__, __FUNCTION__, __LINE__, "Mutex init has failed.");
	    return S_FALSE;
	}
    ...
    unsigned int dwOptions) noexcept
{
    ...
    //DPF(1, "Initialize...");
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Initialize.");
    ...
    std::cout << "/// \033[1;33mSTATUS\033[0m: Initially having " << m_initChartList.size() << " separated charts" << std::endl;
...
LEnd:
    ...
    std::cout << "/// \033[1;32mTIME init\033[0m: " << float( std::clock () - begin_time ) /  CLOCKS_PER_SEC << " sec" << std::endl;
    return hr;
}
    ...
    uint32_t* pFaceAttributeIDOut) noexcept
{
	const std::clock_t begin_time = std::clock();
    //DPF(1, "Partition....");
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Partition.");
    ...
    std::cout << "/// \033[1;32mTIME parti\033[0m: " << float( std::clock () - begin_time ) /  CLOCKS_PER_SEC << " sec" << std::endl;
    return hr;
}
...
bool CIsochartEngine::IsMaxChartNumberValid(
    size_t MaxChartNumber)
{
    if (MaxChartNumber != 0 && (MaxChartNumber < m_initChartList.size() || MaxChartNumber > m_baseInfo.dwFaceCount))
    {
        return false;
    }
    return true;
...
HRESULT CIsochartEngine::ParameterizeChartsInHeap(bool bFirstTime,size_t MaxChartNumber)
{
    // 3.1 If Any charts needed to be partitioned
    while (!m_currentChartHeap.empty())
    {
        //DPF(1, "Processed charts number is : %zu", m_finalChartList.size() + m_currentChartHeap.size());
        std::string message = "Processed charts number is : " + std::to_string(m_finalChartList.size() + m_currentChartHeap.size());
...
HRESULT CIsochartEngine::GenerateNewChartsToParameterize()
{
    ...
    {
        float fMaxStretch;
        dwMaxIdx = CIsochartMesh::GetChartWidthLargestGeoAvgStretch(m_finalChartList, fMaxStretch);
    }
    else
    {
        dwMaxIdx = CIsochartMesh::GetBestPartitionCanidate(m_finalChartList);
    }
    ...
    float& fFinalGeoAvgL2Stretch)
{
    ...
    if (IsIMTSpecified())
    {
        ...
        printProgress(__FILE__, __FUNCTION__, __LINE__, "Begin to optimize signal stretch.");
        // Convert the input stretch to internal stretch,
        // When optimizeing IMT, more stretch is acceptable.
        CIsochartMesh::ConvertToInternalCriterion(Stretch, fExpectAvgL2SquaredStretch, true);
        ...
        FAILURE_RETURN(CIsochartMesh::OptimizeAllL2SquaredStretch(m_finalChartList,true));

        // computer geometric stretch after optimize signal stretch
        CIsochartMesh::ComputeGeoAvgL2Stretch(m_finalChartList,true);
        ...
        fCurrAvgL2SquaredStretch = CIsochartMesh::CalOptimalAvgL2SquaredStretch(m_finalChartList);

        FAILURE_RETURN(CIsochartMesh::OptimalScaleChart(m_finalChartList, fCurrAvgL2SquaredStretch, true));

        // Compute final geometric stretch
        fCurrAvgL2SquaredStretch = CIsochartMesh::ComputeGeoAvgL2Stretch(m_finalChartList, false);
    }
    else
    {
        fCurrAvgL2SquaredStretch = CIsochartMesh::CalOptimalAvgL2SquaredStretch(m_finalChartList);
        FAILURE_RETURN(CIsochartMesh::OptimalScaleChart(m_finalChartList, fCurrAvgL2SquaredStretch, false));
    }
...
float CIsochartEngine::GetCurrentStretchCriteria()
{
    ...
    {
        float fMaxStretch = 0;
        CIsochartMesh::GetChartWidthLargestGeoAvgStretch(m_finalChartList, fMaxStretch);

        return fMaxStretch;
    }
    else
    {
        return CIsochartMesh::CalOptimalAvgL2SquaredStretch(m_finalChartList);
    }
}
...
{
	const std::clock_t begin_time1 = std::clock();
    HRESULT hr = S_OK;

    // 1.  Check current state and parameter.
    if (ISOCHART_ST_UNINITILAIZED == m_state)
    {
    	printError(E_UNEXPECTED, __FILE__, __FUNCTION__, __LINE__);
        return E_UNEXPECTED;
    }
    ...
    CIsochartMesh::ConvertToInternalCriterion(Stretch, fExpectAvgL2SquaredStretch, false);
    ...
    std::string message4 = "Initial chart number " + std::to_string(m_currentChartHeap.size());
    printProgress(__FILE__, __FUNCTION__, __LINE__, message4);
    std::cout << "/// \033[1;32mTIME before loop\033[0m: " << float( std::clock () - begin_time1 ) /  CLOCKS_PER_SEC << " sec" << std::endl;
...
//#endif
        ...
        std::string message2 = "Current charts number is " + std::to_string(m_finalChartList.size());
        printProgress(__FILE__, __FUNCTION__, __LINE__, message2);

        // 3.2 Optimize all charts with right parameterization
        // chart 2d area will be computed in this function
        FAILURE_RETURN(CIsochartMesh::OptimizeAllL2SquaredStretch(m_finalChartList,false));
        ...
        if (dwExpectChartCount != 0)
        {
                ...
                && !bHasSatisfiedNumber)
            {
                ...
                MaxChartStretchOut = CIsochartMesh::ConvertToExternalStretch(fCurrAvgL2SquaredStretch, false);
                //DPF(0, "maximum chart number is too small to parameterize mesh.");
                printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "Maximum chart number is too small to parameterize mesh.");
                return E_FAIL;
            }
        ...
        {
            FAILURE_RETURN(GenerateNewChartsToParameterize());
        }
    ...
    std::cout << "/// \033[1;32mTIME loop\033[0m: " << float( std::clock () - begin_time ) /  CLOCKS_PER_SEC << " sec" << std::endl;


    const std::clock_t begin_time2 = std::clock();
    ...
    std::cout << "/// \033[1;32mTIME FinishWorkAdapt\033[0m: " << float( std::clock () - begin_time2 ) /  CLOCKS_PER_SEC << " sec" << std::endl;

    // 4. MergeChart
    const std::clock_t begin_time3 = std::clock();
    if (m_finalChartList.size() > dwExpectChartCount)
    {
        //DPF(0, "Charts before merge %zu", m_finalChartList.size());
        std::string message = "Charts before merge " + std::to_string(m_finalChartList.size());
        printProgress(__FILE__, __FUNCTION__, __LINE__, message);
        ...
        std::string message2 = "Charts after merge " + std::to_string(m_finalChartList.size());
        printProgress(__FILE__, __FUNCTION__, __LINE__, message2);
    ...
    std::cout << "/// \033[1;32mTIME MergeChart\033[0m: " << float( std::clock () - begin_time3 ) /  CLOCKS_PER_SEC << " sec" << std::endl;

    // 5. Optimize parameterized charts.
    const std::clock_t begin_time4 = std::clock();
    FAILURE_RETURN(OptimizeParameterizedCharts(Stretch, fCurrAvgL2SquaredStretch));
    std::cout << "/// \033[1;32mTIME Optimize\033[0m: " << float( std::clock () - begin_time4 ) /  CLOCKS_PER_SEC << " sec" << std::endl;

    const std::clock_t begin_time5 = std::clock();
    // 6. Export current partition result by set the attribute id of each face 
    // in original mesh	
    if (pFaceAttributeIDOut)
    {
        hr = ExportCurrentCharts(m_finalChartList,pFaceAttributeIDOut);
    }

    ChartNumberOut = m_finalChartList.size();
    MaxChartStretchOut =CIsochartMesh::ConvertToExternalStretch(fCurrAvgL2SquaredStretch,IsIMTSpecified());

    // detect closed surfaces which have not been correctly partitioned.
    for (size_t i = 0; i < m_finalChartList.size(); ++i)
    {
        if (m_finalChartList[i]->GetVertexNumber() > 0 && !m_finalChartList[i]->HasBoundaryVertex())
        {
//        	std::cout << "m_finalChartList[i]->GetVertexNumber(): " << m_finalChartList[i]->GetVertexNumber() << std::endl;
//        	std::cout << "m_finalChartList[i]->HasBoundaryVertex(): " << m_finalChartList[i]->HasBoundaryVertex() << std::endl;
            //DPF(0, "UVAtlas Internal error: Closed surface not correctly partitioned");
            printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "UVAtlas Internal error: Closed surface not correctly partitioned");
            return E_FAIL;
        }
    }
    std::cout << "/// \033[1;32mTIME close surface\033[0m: " << float( std::clock () - begin_time5 ) /  CLOCKS_PER_SEC << " sec" << std::endl;
    ...
    _In_opt_ std::vector<uint32_t>* pvAttributeID) noexcept
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Packing Charts.");
    ...
    if (ISOCHART_ST_PARTITIONED != m_state)
    {
        ...
        printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "Need to partition.");
        return E_FAIL;
    }
...
//   returns    S_OK if successful, else failure code
//
HRESULT CIsochartEngine::SetCallback(LPISOCHARTCALLBACK pCallback, float Frequency) noexcept
{
    if (!CheckSetCallbackParameters(pCallback, Frequency))
    {
    	printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__);
        return E_INVALIDARG;
    }
...
HRESULT CIsochartEngine::SetStage(unsigned int TotalStageCount, unsigned int DoneStageCount) noexcept
{
    ...
    std::vector<CIsochartMesh*>& finalChartList)
{
    ...
    for (size_t i = 0; i < finalChartList.size(); i++)
    {
        ...
        for (size_t j = 0; j < pChart->GetVertexNumber(); j++)
        {
            ...
            pChartVertexBuffer[j].uv.x = pVertexIn->uv.x; // @suppress("Field cannot be resolved")
            pChartVertexBuffer[j].uv.y = pVertexIn->uv.y; // @suppress("Field cannot be resolved")
        }
    ...
    bool bIsForPartition)
{
    ...
    if (FAILED(hr))
    {
        if (hr != E_OUTOFMEMORY)
        {
        	...
        	printError(hr, __FILE__, __FUNCTION__, __LINE__, "Build Full Connection Faild, Non-manifold.");
            delete pRootChart;
            return hr;
        }
        else
    ...
    m_callbackSchemer.InitCallBackAdapt(baseInfo.dwVertexCount * 2 + pRootChart->GetEdgeNumber(), 0.9f, 0.10f);
    ...
    while (!m_currentChartHeap.empty())
    {
        ...
        std::string message = "Separate to " + std::to_string(pChart->GetChildrenCount()) + " sub-charts.";
        printProgress(__FILE__, __FUNCTION__, __LINE__, message);
    ...
    std::string outStr1 = "Old Vert Number is " + std::to_string(baseInfo.dwVertexCount) + ", New Vert Number is " + std::to_string(dwTestVertexCount);
    std::string outStr2 = "Old Face Number is " + std::to_string(baseInfo.dwFaceCount) + ", New Vert Number is " + std::to_string(dwTestFaceCount);
    printProgress(__FILE__, __FUNCTION__, __LINE__, outStr1);
    printProgress(__FILE__, __FUNCTION__, __LINE__, outStr2);
    ...
    std::vector<uint32_t>* pvAdjacencyOut)
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Export Isochart Result...");
    ...
    hr = FillExportVertexBuffer(finalChartList,notUsedVertList,pvVertexArrayOut,pvVertexRemapArrayOut);
    ...
    {
        hr = FillExportFaceIndexBuffer<uint16_t>( finalChartList, pvFaceIndexArrayOut);
    }
    else
    {
        hr = FillExportFaceIndexBuffer<uint32_t>( finalChartList, pvFaceIndexArrayOut);
    }
    if (FAILED(hr = m_callbackSchemer.UpdateCallbackAdapt(1)))
    {
    	printError(hr, __FILE__, __FUNCTION__, __LINE__);
    ...
    if (pvAttributeIDOut)
    {
        if (FAILED(hr = FillExportFaceAttributeBuffer(finalChartList, pvAttributeIDOut)))
        {
        	printError(hr, __FILE__, __FUNCTION__, __LINE__);
            goto LFail;
        }
    ...
    if (pvAdjacencyOut)
    {
        if (FAILED(hr = FillExportFaceAdjacencyBuffer(finalChartList,pvAdjacencyOut)))
        {
        	printError(hr, __FILE__, __FUNCTION__, __LINE__);
            goto LFail;
        }
...
LFail:
    ...
    if (pvAttributeIDOut){ pvAttributeIDOut->clear(); }
    return hr;
}
    ...
    std::vector<uint32_t>* pvAdjacencyOut)
{
    ...
    try
    {
        ...
        if (DXGI_FORMAT_R16_UINT == m_baseInfo.IndexFormat && dwVertCount > 0x0000ffff)
        {
            //DPF(0, "Resulting mesh is too large to fit in 16-bit mesh.");
        	printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "Resulting mesh is too large to fit in 16-bit mesh.");
            return E_FAIL;
        }
    ...
    std::vector<uint32_t>* pvMapBuffer)
{
    ...
    auto pVertex = reinterpret_cast<const uint8_t*>(m_baseInfo.pVertexArray);
    assert(m_baseInfo.dwVertexStride >= sizeof(/*XMFLOAT3*/ glm::vec3));
    ...
    for (size_t i = 0; i < finalChartList.size(); i++)
    {
        ...
        for (size_t j = 0; j < pChart->GetVertexNumber(); j++)
        {
            auto pVertexIn = reinterpret_cast<const /*XMFLOAT3**/ glm::vec3*>(
                static_cast<const void*>(
                    pVertex +
                    m_baseInfo.dwVertexStride * pChartVertexBuffer[j].dwIDInRootMesh));

            *pdwMap = pChartVertexBuffer[j].dwIDInRootMesh;
            pVertexOut->pos.x = pVertexIn->x; // @suppress("Field cannot be resolved")
            pVertexOut->pos.y = pVertexIn->y; // @suppress("Field cannot be resolved")
            pVertexOut->pos.z = pVertexIn->z; // @suppress("Field cannot be resolved")
            pVertexOut->uv.x = pChartVertexBuffer[j].uv.x; // @suppress("Field cannot be resolved")
            pVertexOut->uv.y = pChartVertexBuffer[j].uv.y; // @suppress("Field cannot be resolved")
    ...
    for (size_t ii = 0; ii < notUsedVertList.size(); ii++)
    {
        ...
        auto pVertexIn = static_cast<const /*XMFLOAT3**/ glm::vec3*>(
            static_cast<const void*>(
                pVertex +
        ...
        *pdwMap = dwIDInOriginalMesh;
        ...
        pVertexOut->pos.y = pVertexIn->y; // @suppress("Field cannot be resolved")
        pVertexOut->pos.z = pVertexIn->z; // @suppress("Field cannot be resolved")
        pVertexOut->uv.x = 1.0f; // @suppress("Field cannot be resolved")
        pVertexOut->uv.y = 1.0f; // @suppress("Field cannot be resolved")
        pVertexOut++;
        pdwMap++;
    }
    ...
    std::vector<uint8_t>* pvFaceBuffer)
{
    ...
    for (size_t i = 0; i < finalChartList.size(); i++)
    {
        ...
        for (size_t j = 0; j < pChart->GetFaceNumber(); j++)
        {
            ...
            pFaces[0] = static_cast<INDEXTYPE>(pChartFaceBuffer[j].dwVertexID[0] + dwOffset);

            pFaces[1] = static_cast<INDEXTYPE>(pChartFaceBuffer[j].dwVertexID[1] + dwOffset);

            pFaces[2] = static_cast<INDEXTYPE>(pChartFaceBuffer[j].dwVertexID[2] + dwOffset);
            ...
            for (size_t k = 0; k < 3; k++)
            {
                ...
                if (pEdge.bIsBoundary)
                {
                    ...
                    if (!pEdge.bCanBeSplit)
                    {
                    	...
                    	printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "UVAtlas Internal error: Made non-splittable edge a boundary edge.");
                        return E_FAIL;
                    }
                }
...
HRESULT CIsochartEngine::TryEnterExclusiveSection()
{
    ...
    if (pthread_mutex_lock(&m_hMutex) == 0)//(WaitForSingleObjectEx(m_hMutex, 0, FALSE) == WAIT_OBJECT_0)
    {
        return S_OK;
    }
    else
...
bool Isochart::CheckSetCallbackParameters(LPISOCHARTCALLBACK pCallback,float Frequency)
{
