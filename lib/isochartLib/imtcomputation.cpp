...
namespace
{
    ...
    static bool CheckIMTFromTextureMapInput(
        const /*XMFLOAT3**/ glm::vec3* pV3d,	// [In] surface coordinates of face's vertices
        const /*XMFLOAT2**/ glm::vec2* pUV,	// [In] Texture coordinates of each vertices in the range of 0 to 1.0f
    ...
    static void ConvertToCanonicalIMT(
        ...
        float* pfIMT,
        const /*XMFLOAT3**/ glm::vec3* pV3d,
        const /*XMFLOAT2**/ glm::vec2* pUV)
    {
        float fOldA = Cal2DTriangleArea(pUV, pUV + 1, pUV + 2);
        ...
        /*XMFLOAT2*/ glm::vec2 v2d[3];
        /*XMFLOAT3*/ glm::vec3 axis[2]; // X, Y axis
        ...
        float fNewA = Cal2DTriangleArea(v2d, v2d + 1, v2d + 2);
    ...
    static bool IsContinueSplit(
        ...
        SUBFACE* pFace,
        std::vector</*XMFLOAT2*/glm::vec2>& vertList)
    {
        ...
        for (size_t ii = 0; ii < 3; ii++)
        {
        	glm::vec4 v = glm::vec4(texCoord[ii], 0.0f, 0.0f) - glm::vec4(texCoord[(ii+1)%3], 0.0f, 0.0f);
            float fDisc = glm::length(glm::vec2(v.x,v.y));
            //XMVECTOR v = XMLoadFloat2(&texCoord[ii]) - XMLoadFloat2(&texCoord[(ii + 1) % 3]);
            //float fDisc = XMVectorGetX(XMVector2Length(v));
    ...
    HRESULT SplitFace(
        ...
        /*std::vector<XMFLOAT2>&*/ std::vector<glm::vec2>& vertList)
    {
        HRESULT hr = S_OK;
        /*XMFLOAT2*/ glm::vec2 vNew;
        ...
        try
        {
            for (size_t ii = 0; ii < 3; ii++)
            {
            	glm::vec2 vNew = (glm::vec2(vertList[pFace->dwVertIdx[ii]] + vertList[pFace->dwVertIdx[(ii + 1) % 3]]) / 2.0f);
                /*XMStoreFloat2(&vNew,
    ...
    FLOAT3* pfIMTArray)
{
    ...
    float f2D = Cal2DTriangleArea(v2d, v2d + 1, v2d + 2);
...
//-------------------------------------------------------------------------------------
HRESULT Isochart::IMTFromTextureMap(
    const /*XMFLOAT3**/ glm::vec3* pV3d,	// [In] surface coordinates of face's vertices
    const /*XMFLOAT2**/ glm::vec2* pUV,	// [In] Texture coordinates of each vertices in the range of 0 to 1.0f
    ...
    FLOAT3* pfIMTArray)	// [Out] Result IMT
{
    ...
    double d3dArea = fabs(double(Cal3DTriangleArea(pV3d, pV3d + 1, pV3d + 2)));
    double d2dArea = fabs(double(Cal2DTriangleArea(pUV, pUV + 1, pUV + 2)));

    if (IsInZeroRangeDouble(d3dArea) ||
        IsInZeroRangeDouble(d2dArea))
    {
        //DPF(0, "IMTFromTextureMap failed due to zero area");
        printError(hr, __FILE__, __FUNCTION__, __LINE__, "IMTFromTextureMap failed due to zero area.");
        return E_FAIL;
    }
    ...
    {
        /*XMFLOAT2*/ glm::vec2 coord = vertList[ii];
...
//-------------------------------------------------------------------------------------
HRESULT Isochart::IMTFromTextureMapEx(
    const /*XMFLOAT3**/ glm::vec3* pV3d,	// [In] surface coordinates of face's vertices
    const /*XMFLOAT2**/ glm::vec2* pUV,	// [In] Texture coordinates of each vertices in the range of 0 to 1.0f
    size_t uPrimitiveId,
    size_t dwSignalDimension,
    ...
    FLOAT3* pfIMTArray)	// [Out] Result IMT
{
    ...
    std::string message1 = "2d area by formal " + std::to_string(double(f2dArea));
    std::string message2 = "Integrated 2d area " + std::to_string(dTotal2DArea);
    ...
    ConvertToCanonicalIMT((*pfIMTArray),(*pfIMTArray),pV3d,pUV);
