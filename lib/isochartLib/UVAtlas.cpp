...
#pragma warning( suppress : 4127 )
        if ((sizeof(IndexType) == sizeof(uint16_t)) && (*nNewVerts > 0x0fffe))
        {
            ...
            printError(ERROR_INVALID_DATA, __FILE__, __FUNCTION__, __LINE__, "Resulting mesh is too large to fit in 16-bit mesh.");
        ...
        _In_                        size_t nFaces)
    {
        ...
        if ((uint64_t(nFaces) * 3) >= UINT32_MAX)
            return ERROR_ARITHMETIC_OVERFLOW/*HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW)*/;
        ...
        for (size_t i = 0; i < nFaces; i++)
        {
            for (size_t j = 0; j < 3; j++)
            {
                ...
                if (k >= 3)
                {
                    ...
                    std::string message = "Adjacency data is invalid, " + std::to_string(neighbor) + " is a neighbor of " + std::to_string(i) + ", but not vice versa.";
                    printError(ERROR_INVALID_DATA, __FILE__, __FUNCTION__, __LINE__, message);
                    return /*HRESULT_FROM_WIN32(ERROR_INVALID_DATA)*/ERROR_INVALID_DATA;
                }
        ...
        for (size_t i = 0; i < nFaces; i++)
        {
            for (size_t j = 0; j < 3; j++)
            {
                ...
                if (FindEquivParent(pEquivs, v1) != FindEquivParent(pEquivs, v2))
                {
                    ...
                    std::string message = "False edge data is invalid, " + std::to_string(pIndexData[v1]) + " and " + std::to_string(pIndexData[v2]) + " are only connected by false edges.";
                    printError(ERROR_INVALID_DATA, __FILE__, __FUNCTION__, __LINE__, message);
                    //return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
                    return ERROR_INVALID_DATA;
                }
    ...
    HRESULT UVAtlasPartitionInt(
        _In_reads_(nVerts)          const /*XMFLOAT3**/ glm::vec3* positions,
        _In_                        size_t nVerts,
        ...
        _In_                        unsigned int uStageInfo)
    {
        ...
        if (!adjacency)
        {
            ...
            printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__, "Input adjacency pointer cannot be nullptr. Use DirectXMesh to compute it");
            return E_INVALIDARG;
        }
        ...
        if (falseEdgeAdjacency)
        {
            for (size_t i = 0; i < 3 * nFaces; i++)
            {
                    ...
                    (falseEdgeAdjacency[i] != uint32_t(-1)))
                {
                    ...
                    printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__, "False edge found on triangle with no adjacent triangle.");
                    return ERROR_INVALID_DATA;
                    //return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
                }
            }
        ...
        // copy old vertex data using remap array
        {
            ...
            for (size_t i = 0; i < outMeshNumVertices; i++)
            {
                ...
                memcpy(&bBaseOut[i].pos.x, bBaseIn + pdwRemap[i] * sizeof(glm::vec3), sizeof(glm::vec3));
        ...
        _In_                    unsigned int uStageInfo)
    {
        ...
        if ((uint64_t(nFaces) * 3) >= UINT32_MAX)
        {
        	printError(ERROR_ARITHMETIC_OVERFLOW, __FILE__, __FUNCTION__, __LINE__);
        	return ERROR_ARITHMETIC_OVERFLOW;
        }
            //return HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW);
        ...
        if (vPartitionResultAdjacency.size() != (nFaces * 3))
        {
            ...
            printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__, "Partition result adjacency info invalid");
            return E_INVALIDARG;
        }
...
HRESULT /*__cdecl DirectX::*/UVAtlasPartition(
    const /*XMFLOAT3**/ glm::vec3* positions,
    size_t nVerts,
    const void* indices,
...
//		std::cout << adjacency[i] << ", " << adjacency[i+1] << ", " << adjacency[i+2] << std::endl;
//	}

//	auto facess = reinterpret_cast <const uint32_t*>(indices);
//	for(unsigned int i = 0; i < nFaces*3; i+=3){
//		std::cout << facess[i] << ", " << facess[i+1] << ", " << facess[i+2] << std::endl;
//	}
    ...
    std::cout << "/// \033[1;32mTIME Partition\033[0m: " << float( std::clock () - begin_time ) /  CLOCKS_PER_SEC << " sec" << std::endl;
    ...
    std::cout << "/// \033[1;32mTIME Pack\033[0m: " << float( std::clock () - begin_time2 ) /  CLOCKS_PER_SEC << " sec" << std::endl;
    ...
    float* pIMTArray) noexcept
{
    ...
    if (!signalStride || (signalStride % sizeof(float)))
    {
        ...
        std::string message = "UVAtlasComputeIMT: signalStride (" + std::to_string(signalStride) + ") must be a multiple of " + std::to_string(sizeof(float));
        printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__, message);
        return E_INVALIDARG;
    }
    ...
    if ((signalStride / sizeof(float)) < signalDimension)
    {
        ...
        std::string message = "UVAtlasComputeIMT: signalStride (" + std::to_string(signalStride) + ") must accommodate signal dimension float values (" + std::to_string(signalDimension) + ")";
        printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__, message);
        return E_INVALIDARG;
    }
    ...
    for (size_t i = 0; i < nFaces; i++)
    {
        ...
        for (size_t j = 0; j < 3; j++)
        {
            ...
            if (dwId >= nVerts)
            {
                ...
                printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "UVAtlasComputeIMT: Vertex ID out of range.");
                return E_FAIL;
            }
        ...
        if (FAILED(hr))
        {
            ...
            printError(hr, __FILE__, __FUNCTION__, __LINE__, "UVAtlasComputeIMT: IMT data calculation failed.");
            return hr;
        }
    }
    ...
    if (statusCallBack)
    {
        ...
        if (FAILED(hr))
        {
            printError(E_ABORT, __FILE__, __FUNCTION__, __LINE__);
            return E_ABORT;
        }
    }
...
HRESULT /*__cdecl DirectX::*/UVAtlasComputeIMTFromSignal(
    const /*XMFLOAT3**/ glm::vec3* positions,
    const /*XMFLOAT2**/ glm::vec2* texcoords,
    size_t nVerts,
    const void* indices,
    DXGI_FORMAT indexFormat,
    size_t nFaces,
    size_t signalDimension,
    float maxUVDistance,
    std::function<HRESULT /*__cdecl*/(const /*DirectX::XMFLOAT2*/ glm::vec2 * uv, size_t primitiveID, size_t signalDimension, void* userData, float* signalOut)> signalCallback,
    void* userData,
    std::function<HRESULT /*__cdecl*/(float percentComplete)> statusCallBack,
    float* pIMTArray) noexcept
{
    ...
    if (!signalCallback)
    {
        ...
        printError(E_ABORT, __FILE__, __FUNCTION__, __LINE__, "ComputeIMTFromSignal: requires signal computation callback.");
        return E_INVALIDARG;
    }
...
namespace
{
    ...
    {
        const /*XMFLOAT4**/ glm::vec4* pTexture;
        size_t uHeight, uWidth;
    };

    HRESULT /*__cdecl*/ IMTTextureCbWrapNone(const /*XMFLOAT2**/ glm::vec2* uv,
        size_t uPrimitiveId,
        ...
        float* pfSignalOut)
    {
        ...
        /*XMVECTOR*/ glm::vec4 C1, C2, C3, C4, res;
        ...
        C1 = pTexDesc->pTexture[j * int(pTexDesc->uWidth) + i];
        C2 = pTexDesc->pTexture[j * int(pTexDesc->uWidth) + i2];
        C3 = pTexDesc->pTexture[j2 * int(pTexDesc->uWidth) + i];
        C4 = pTexDesc->pTexture[j2 * int(pTexDesc->uWidth) + i2];

        res = (C1 * (1.f - du) + C2 * du) * (1.f - dv) + (C3 * (1.f - du) + C4 * du) * dv;

        //XMStoreFloat4(reinterpret_cast<XMFLOAT4*>(pfSignalOut), res);
        glm::vec4* pfSignalOutTemp = reinterpret_cast<glm::vec4*>(pfSignalOut);
        pfSignalOutTemp = &res;

        return S_OK;
    }

    HRESULT /*__cdecl*/ IMTTextureCbWrapU(const /*XMFLOAT2**/ glm::vec2* uv,
        size_t uPrimitiveId,
        size_t uSignalDimension,
        void* pUserData,
        float* pfSignalOut)
    {
        ...
        C3 = XMLoadFloat4(&pTexDesc->pTexture[j2 * int(pTexDesc->uWidth) + i]);
        C4 = XMLoadFloat4(&pTexDesc->pTexture[j2 * int(pTexDesc->uWidth) + i2]);*/
        glm::vec4 C1, C2, C3, C4, res;
        C1 = pTexDesc->pTexture[j * int(pTexDesc->uWidth) + i];
        C2 = pTexDesc->pTexture[j * int(pTexDesc->uWidth) + i2];
        C3 = pTexDesc->pTexture[j2 * int(pTexDesc->uWidth) + i];
        C4 = pTexDesc->pTexture[j2 * int(pTexDesc->uWidth) + i2];

        res = (C1 * (1.f - du) + C2 * du) * (1 - dv) + (C3 * (1.f - du) + C4 * du) * dv;
    ...
    HRESULT /*__cdecl*/ IMTTextureCbWrapV(const /*XMFLOAT2**/glm::vec2* uv,
        size_t uPrimitiveId,
        size_t uSignalDimension,
        void* pUserData,
        float* pfSignalOut)
    ...
    HRESULT /*__cdecl*/ IMTTextureCbWrapUV(const /*XMFLOAT2**/glm::vec2* uv,
        size_t uPrimitiveId,
        size_t uSignalDimension,
        void* pUserData,
        float* pfSignalOut)
    ...
    float* pIMTArray) noexcept
{
    ...
    TextureDesc.pTexture = reinterpret_cast<const /*XMFLOAT4**/ glm::vec4*>(pTexture);
...
namespace
{
    ...
    HRESULT /*__cdecl*/ IMTFloatArrayCbWrapNone(const /*XMFLOAT2**/ glm::vec2* uv,
        size_t uPrimitiveId,
        size_t uSignalDimension,
        void* pUserData,
        float* pfSignalOut)
    ...
    HRESULT /*__cdecl*/ IMTFloatArrayCbWrapU(const /*XMFLOAT2**/ glm::vec2* uv,
        size_t uPrimitiveId,
        size_t uSignalDimension,
        void* pUserData,
        float* pfSignalOut)
    ...
    HRESULT /*__cdecl*/ IMTFloatArrayCbWrapV(const /*XMFLOAT2**/ glm::vec2* uv,
        size_t uPrimitiveId,
        size_t uSignalDimension,
        void* pUserData,
        float* pfSignalOut)
    ...
    HRESULT /*__cdecl*/ IMTFloatArrayCbWrapUV(const /*XMFLOAT2**/ glm::vec2* uv,
        size_t uPrimitiveId,
        size_t uSignalDimension,
        void* pUserData,
        float* pfSignalOut)
    ...
    float* pIMTArray) noexcept
{
    ...
    if (nComponents < signalDimension)
    {
        ...
        printError(E_INVALIDARG, __FILE__, __FUNCTION__, __LINE__, "UVAtlasComputeIMT: number of components must be >= signal dimension");
        return E_INVALIDARG;
    }
    ...
    void* vbout) noexcept
{
    ...
    if (vbin == vbout)
    {
    	printError(ERROR_NOT_SUPPORTED, __FILE__, __FUNCTION__, __LINE__);
