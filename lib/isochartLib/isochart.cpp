    ...
    unsigned int dwOptions)
{
    ...
    std::string message1 = "Vertex Number: " + std::to_string(VertexCount);
    std::string message2 = "Face Number: " + std::to_string(FaceCount);
    ...
    if (pCallback)
    {
        if (FAILED(hr = pEngine->SetCallback(pCallback,Frequency)))
        {
        	printError(hr, __FILE__, __FUNCTION__, __LINE__);
            goto LEnd;
        }
...
//    else
//        std::cout << "/// \033[1;32mTIME parti\033[0m: " << float( std::clock () - begin_time2 ) /  CLOCKS_PER_SEC << " sec" << std::endl;
...
LEnd:
    ...
    if (pChartNumberOut){ *pChartNumberOut = dwChartNumberOut; }

    if (pMaxStretchOut){ *pMaxStretchOut = fMaxChartStretchOut; }
