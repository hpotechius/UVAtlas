    ...
    float& fAverageAngleDistance) const
{
    ...
    for (size_t i = 0; i < m_dwEdgeNumber; i++)
    {
        ...
        if (!edge.bIsBoundary)
        {
            ...
            pfEdgeAngleDistance[i] = glm::dot(*(m_baseInfo.pFaceNormalArray+pFace1->dwIDInRootMesh), *(m_baseInfo.pFaceNormalArray+pFace2->dwIDInRootMesh));
    ...
    bool& bIsOptimized)
{
    ...
    if (FAILED(hr = MakePartitionValid(
        ...
        bIsOptimized)))
    {
        if (hr == ERROR_INVALID_DATA/*HRESULT_FROM_WIN32(ERROR_INVALID_DATA)*/)
        {
            bIsOptimized = false;
            return hr = S_OK;
        }
