    ...
    bool bIsSignalSpecialized)
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Convert Stretch.");
            ...
            for (size_t j = 0; j < 3; j++)
            {
                ...
                {	// at least 3 faces have the same edge, non-manifold
                    if (pEdge->dwFaceID[1] != INVALID_FACE_ID)
                    {
                        ...
                        printProgress(__FILE__, __FUNCTION__, __LINE__, "Non-manifold: More than 2 faces have the same edge.");
                        return S_OK;
                    }
...
HRESULT CIsochartMesh::CleanNonmanifoldMesh(bool& bCleaned)
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Try to clean the non-manifold mesh, generated by partition.");
        ...
        for (size_t jj = 0; jj < 3; jj++)
        {
            if (pSplitInfo[jj] != INVALID_FACE_ID)
            {
                if (m_edges[face.dwEdgeID[jj]].bIsBoundary)
                {
                    ...
                    printError(E_FAIL, __FILE__, __FUNCTION__, __LINE__, "UVAtlas Internal error: Non-splittable edge was chosen as a boundary edge.");
                    return E_FAIL;
                }
...
bool CIsochartMesh::IsAllFaceVertexOrderValid()
{
    ...
    for (size_t i = 0; i < m_dwEdgeNumber; i++)
    {
        ...
        {
            DPF(3, "Non-manifold: The mesh is folded. 2 face on the same side of a edge...\n");
            printProgress(__FILE__, __FUNCTION__, __LINE__, "Non-manifold: The mesh is folded. 2 face on the same side of a edge.");
            return false;
        }
    }
    return true;
    ...
    ISOCHARTVERTEX* pVertex)
{
    ...
    if (!pCurrentEdge)
    {
        //DPF(3, "Non-manifold: A boundary vertex doesn't has adjacent boundary edge.\n");
        printProgress(__FILE__, __FUNCTION__, __LINE__, "Non-manifold: A boundary vertex doesn't has adjacent boundary edge.");
        return false;
    }
    ...
    if (pTriangle->dwVertexID[(k + 1) % 3] != dwNextV)
    {
        ...
        if (!pCurrentEdge)
        {
            ...
            printProgress(__FILE__, __FUNCTION__, __LINE__, "Non-manifold: A boundary vertex only has one adjacent boundary edge.");
            return false;
        }
        if (pCurrentEdge->dwVertexID[0] == pVertex->dwID)
        {
        ...
        if (m >= 3 || pTriangle->dwVertexID[(m + 1) % 3] != dwNextV)
        {
            ...
            printProgress(__FILE__, __FUNCTION__, __LINE__, "Non-manifold: logic error, Need to be investigated.");
            return false;
        }
    }
    ...
    try
    {
        for (size_t j = 0; j < dwEdgeNum; j++)
        {
            if (pCurrentEdge == pPreEdge)
            {
                ...
                printProgress(__FILE__, __FUNCTION__, __LINE__, "Non-manifold: Vertex has more than 2 adjacent boundary edges.");
                return false;
            }
    ...
    try
    {
        ...
        for (size_t j = 1; j < dwEdgeNum; j++)
        {
            ...
            if (k < j)
            {
                ...
                printProgress(__FILE__, __FUNCTION__, __LINE__, "Non-manifold: Vertex has two same adjacent vertices.");
                return false;
            }
    ...
    try
    {
        for (size_t i = 0; i < m_dwVertNumber; i++)
        {
            ...
            else if (vertList.size() > 0)
            {
                ...
                std::string message1 = "Generate new mesh: " + std::to_string(pChart->m_dwVertNumber) + " vert, "
                		 + std::to_string(pChart->m_dwFaceNumber) + " face, "
						 + std::to_string(pChart->m_dwEdgeNumber) + " edge.";
                printProgress(__FILE__, __FUNCTION__, __LINE__, message1);
            }
        }
        //DPF(3, "....Divide into %zu sub-meshes...\n", m_children.size());
        std::string message2 = "Divide into " + std::to_string(m_children.size()) + " sub-meshes.";
    ...
    size_t& dwBoundaryNumber)
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Check and cut multi boundary.");
    ...
    float& fDistance)
{
    ...
    for (size_t i = 0; i < m_dwVertNumber; i++)
    {
        ...
        for (size_t j = 0; j < pCurrentVertex->vertAdjacent.size(); j++)
        {
            ...
            if (m_baseInfo.pdwSplitHint && !m_edges[pCurrentVertex->edgeAdjacent[j]].bCanBeSplit)
            {
                continue;
            }
    ...
    uint32_t* pdwVertBoundaryID)
{
    ...
    std::string message = "Has: " + std::to_string(dwBoundaryNumber) + " boundies.";
    ...
    FAILURE_RETURN(CutChartAlongPath(minDijkstraPath));
...
HRESULT CIsochartMesh::CutChartAlongPath(std::vector<uint32_t>& dijkstraPath)
{
    ...
    if (SUCCEEDED(hr = pChart->BuildFullConnection(bManifold)))
    {
        if (!bManifold)
        {
            hr = /*HRESULT_FROM_WIN32(ERROR_INVALID_DATA)*/ ERROR_INVALID_DATA;
            goto LEnd;
        }
    ...
    std::vector<uint32_t>& corresVertList)
{
    ...
    std::string message = "New vert number is :" + std::to_string(dwNewVertNumber);
    printProgress(__FILE__, __FUNCTION__, __LINE__, message);
...
HRESULT CIsochartMesh::CalculateVertImportanceOrder()
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Calculate Importance order for each vertex.");
    HRESULT hr = S_OK;
    ...
    std::cout << "/// \033[1;32mTIME VertexImportance\033[0m: " << float( std::clock () - begin_time ) /  CLOCKS_PER_SEC << " sec" << std::endl;
