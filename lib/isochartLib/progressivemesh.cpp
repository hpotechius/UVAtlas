...
HRESULT CProgressiveMesh::Simplify()
{
    ...
    printProgress(__FILE__, __FUNCTION__, __LINE__, "----Begin Simplify----");
    ...
    size_t dwRepeat = 0;
    while (dwEdgeCount < m_dwEdgeNumber && dwRemainVertNumber > dwMinVertNumber)
    {
        ...
        PMISOCHARTEDGE* pCurrentEdge = m_pEdgeArray + pNeedCutEdgeItem->m_data;
        assert(!pCurrentEdge->bIsDeleted);

        // 2.2 If deleting current edge makes distortion more than some limit, then stop
        // deleting edges.

        if (static_cast<float>(fabs(pNeedCutEdgeItem->m_weight)) > fMaxError* m_fBoxDiagLen)
        {
            break;
        }
    ...
    while (dwRemainVertNumber > dwMinVertNumber)
    {
        ...
        if (!PrepareDeletingEdge(pCurrentEdge, &pReserveVertex, &pDeleteVertex, bIsGeodesicValid))
        {
            continue;
        }
    ...
    std::cout << "/// \033[1;32mTIME Simplification\033[0m: " << float( std::clock () - begin_time ) /  CLOCKS_PER_SEC << " sec" << std::endl;

    //DPF(3, "#Remained vert: %d\n", dwRemainVertNumber);
    //DPF(3, "Exported simplified mesh");
    std::string message = "#Remained vert: " + std::to_string(dwRemainVertNumber);
    printProgress(__FILE__, __FUNCTION__, __LINE__, message);
    printProgress(__FILE__, __FUNCTION__, __LINE__, "Exported simplified mesh.");
    ...
    bool& bIsGeodesicValid)
{
    ...
    if (pCurrentEdge->bIsDeleted || pCurrentEdge->dwVertexID[0] == pCurrentEdge->dwVertexID[1])
    {
        return false;
    }
    ...
    // be deleted
    *ppReserveVertex = m_pVertArray + pCurrentEdge->dwVertexID[1 - pCurrentEdge->dwDeleteWhichVertex];

    *ppDeleteVertex = m_pVertArray + pCurrentEdge->dwVertexID[pCurrentEdge->dwDeleteWhichVertex];
    ...
    if (!IsProgressiveMeshToplogicValid(pCurrentEdge, *ppReserveVertex, *ppDeleteVertex))
    {
        return false;
    }
    ...
    PMISOCHARTVERTEX* pDeleteVertex) const
{
    for (size_t k = 0; k < 2; k++)
    {
        ...
        PMISOCHARTVERTEX* pThirdVertex =  m_pVertArray + pEdge->dwOppositVertID[k];
...
bool CProgressiveMesh::IsProgressiveMeshGeometricValid(
    ...
    PMISOCHARTVERTEX* pDeleteVertex) const
{
    /*XMFLOAT3**/ glm::vec3* pv[3];
    /*XMFLOAT3*/ glm::vec3 v1, v2, normal;
    ...
    PMISOCHARTVERTEX* pDeleteVertex)
{
    ...
    for (size_t j = 0; j < pReserveVertex->faceAdjacent.size(); j++)
    {
        ...
        pFace->normal = glm::normalize(pFace->normal);
    }
}
...
HRESULT CProgressiveMesh::CalculateQuadricErrorMetric()
{
    ...
    /*XMFLOAT3*/ glm::vec3 leftTop(
        FLT_MAX,
        FLT_MAX,
        FLT_MAX);

    /*XMFLOAT3*/ glm::vec3 rightBottom(
        -FLT_MAX,
        -FLT_MAX,
        -FLT_MAX);
    ...
    m_fBoxDiagLen = glm::length(rightBottom - leftTop);
...
HRESULT CProgressiveMesh::CalculateQuadricArray()
{
    ...
    /*XMFLOAT3*/ glm::vec3 tempVector;
    /*XMFLOAT3*/ glm::vec3 normal;
    ...
    try
    {
        ...
        for (size_t i = 0; i < m_dwFaceNumber; i++)
        {
            ...
            normal = pFace->normal;
            fTemp = double(IsochartVec3Dot(&normal, m_baseInfo.pVertPosition + pVert->dwIDInRootMesh));
            fTemp = -fTemp;
            /*
    ...
    PMISOCHARTEDGE* pEdge)
{
    ...
    {
        /*XMFLOAT3**/ glm::vec3* pv = m_baseInfo.pVertPosition + pVertex1->dwIDInRootMesh;
        tempVector[0] = pv->x;
        tempVector[1] = pv->y;
        tempVector[2] = pv->z;
    ...
    {
    	/*XMFLOAT3**/ glm::vec3* pv = m_baseInfo.pVertPosition + pVertex2->dwIDInRootMesh;
        tempVector[0] = pv->x;
        tempVector[1] = pv->y;
        tempVector[2] = pv->z;
