...
/*
 * see https://www.leadtools.com/help/sdk/v20/dh/cw/hresult.html for explanation
 */
enum HRESULT{
	S_OK 						= 0x00000000,
	S_FALSE						= 0x00000001,
	...
	ContextNotActive			= 0x80000227,
	ImproperSignatureFormat		= 0x80000226,
	FilterNotSet				= 0x80000225,
	ActionNotAllowed			= 0x80000224,
	ActionNotPossible			= 0x80000223,
	AlreadyJoined				= 0x80000222,
	BadPropertyValue			= 0x80000221,
	BadPropertyType				= 0x80000220,
	UknownPropertyName			= 0x8000021F,
	ImproperMACFormat			= 0x8000021E,
	BindingRejected				= 0x8000021D,
	ImproperKeyFormat			= 0x8000021C,
	UnknownBindee				= 0x8000021B,
	UnknownDataFormat			= 0x8000021A,
	LogonNotFound				= 0x80000219,
	UnknownConnection			= 0x80000218,
	UnknownApplication			= 0x80000217,
	SignatureRequired			= 0x80000216,
	AuthenticationFailed		= 0x80000215,
	ChangesNotAllowed			= 0x80000214,
	ChangesNotPossible			= 0x80000213,
	UndoNotPossible				= 0x80000212,
	InvalidTransaction			= 0x80000211,
	BadItemValue				= 0x80000210,
	BadItemType					= 0x8000020F,
	BadItemNameFormat			= 0x8000020E,
	AcceptNotPossible			= 0x8000020D,
	TooManyParticipants			= 0x8000020C,
	UnknownParticipant			= 0x8000020B,
	UnknownItemName				= 0x8000020A,
	TransactionInProgress		= 0x80000209,
	NotInTransaction			= 0x80000207,
	NameValueCountMismatch		= 0x80000206,
	InvalidContextCoupon		= 0x80000203,
	ChangesNotEnded				= 0x80000201,
	E_FAIL						= 0x00000002,
	E_UNEXPECTED				= 0x00000003,
	ERROR_INVALID_DATA			= 0x00000004,
	E_POINTER					= 0x00000005,
	ERROR_ARITHMETIC_OVERFLOW	= 0x00000006,
	ERROR_NOT_SUPPORTED			= 0x00000007,
	E_ABORT						= 0x00000008
};

bool FAILED(HRESULT hr);
bool SUCCEEDED(HRESULT hr);
void printError(HRESULT hr, std::string file, std::string function, unsigned int line, std::string message = "no message");
void printProgress(std::string file, std::string function, unsigned int line, std::string message = "no message");
...
namespace DirectX
{
    ...
    {
        /*XMFLOAT3*/ glm::vec3 pos;
        /*XMFLOAT2*/ glm::vec2 uv;
    };
    ...
    HRESULT UVAtlasCreate(
        _In_reads_(nVerts)                  /*const XMFLOAT3**/ const glm::vec3* positions,
        _In_                                size_t nVerts,
        _When_(indexFormat == DXGI_FORMAT_R16_UINT, _In_reads_bytes_(nFaces * sizeof(uint16_t)))
        _When_(indexFormat != DXGI_FORMAT_R16_UINT, _In_reads_bytes_(nFaces * sizeof(uint32_t)))
											const void* indices,
    ...
    HRESULT UVAtlasComputeIMTFromSignal(
        _In_reads_(nVerts)                  /*const XMFLOAT3**/ glm::vec3* positions,
        _In_reads_(nVerts)                  /*const XMFLOAT2**/ glm::vec2* texcoords,
        _In_                                size_t nVerts,
        _When_(indexFormat == DXGI_FORMAT_R16_UINT, _In_reads_bytes_(nFaces * sizeof(uint16_t)))
        _When_(indexFormat != DXGI_FORMAT_R16_UINT, _In_reads_bytes_(nFaces * sizeof(uint32_t))) 
						const void* indices,
        ...
        _In_ std::function<HRESULT (/*const XMFLOAT2 **/ glm::vec2* uv, size_t primitiveID, size_t signalDimension, void* userData, float* signalOut)>
        signalCallback,
